...
// src/functions/addPet/handler.ts
__export(exports, {
  addPet: () => addPet,
  main: () => main
});
...
var Dynamo = {
  ...
  async deletePet(id, TableName) {
    ...
    if (!res) {
      throw Error(`There was an error deleting pet ${id} in table ${TableName}`);
    }
  ...
  async addPET(pet, TableName) {
    ...
    if (!res) {
      throw Error(`There was an error inserting ID of ${pet.ID} and email ${pet.name} in table ${TableName}`);
    }
    return pet;
  }
};
...
var table_name = "usersTable_pets_ahsan";

// src/functions/addPet/handler.ts
var addPet = async (event) => {
  let pet = event.body;
  let id = await Math.floor(1e7 + Math.random() * 123456);
  pet.id = id;
